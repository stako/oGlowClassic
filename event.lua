...
eventFrame:SetScript('OnEvent', function(self, event, ...)
	return oGlowClassic[event](oGlowClassic, event, ...)
end)
...
function oGlowClassic:RegisterEvent(event, func)
...
function oGlowClassic:IsEventRegistered(event)
	return eventFrame:IsEventRegistered(event)
end

function oGlowClassic:UnregisterEvent(event, func)
	argcheck(event, 2, 'string')
